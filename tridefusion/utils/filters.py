...
#     Args:
#         noisy_img (np.ndarray): 3D volume to denoise, shape (Z, H, W).
#         h (int): Filter strength.
#         temporal_window (int): Number of slices used for denoising each frame (must be odd).
#         template_window (int): Size in pixels of the template patch.
#         search_window (int): Size in pixels of the window used to compute weighted average.
#     Returns:
#         np.ndarray: Denoised volume with shape (Z, H, W), dtype=float.
#     """
#     try:
#         if img.ndim != 3:
...
#         Z, H, W = img.shape
#         noisy_img_uint8 = np.stack([cv2.convertScaleAbs(img) for img in img], axis=0)
#         denoised = []
#         for i in range(Z):
...
#                 ref_index,
#                 temporalWindowSize=min(temporal_window, end - start),
#                 h=h,
