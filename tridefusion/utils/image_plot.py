...
def draw_3d_img_figure(img: np.ndarray, 
                       output_path: str = None) -> None:
    """
    Draw a 3D minimalistic figure of a NumPy array (e.g., a 3D surface plot) where each slice in the Z-axis is visualized
    ...
    z = np.arange(normalized_img.shape[0])  
    x = np.arange(normalized_img.shape[1]) 
    y = np.arange(normalized_img.shape[2])  
    x, y = np.meshgrid(x, y)
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')

    for i in range(normalized_img.shape[0]):
        z_layer = normalized_img[i, :, :]
    ...
    mappable.set_array(normalized_img)
