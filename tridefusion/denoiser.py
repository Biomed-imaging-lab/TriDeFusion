...
#     @memory_monitor(print_params=["method"])
#     def denoise_image(self, noisy_img: np.ndarray, 
#                       method: str = "gaussian",
#                       normalize_output: bool = True,
#                       save_path: str = None) -> np.ndarray:
#         try:
#             check_not_none(noisy_img, "Noisy image")
...
#         filters_module = importlib.import_module("src.filters")
#         valid_methods = list(self.config.keys())
#         valid_method_name(method=method, method_list=valid_methods, method_type="denoise method")
#         for method, cfg in self.config.items():
#             fn = cfg.get("function")
#             if fn and not hasattr(filters_module, fn):
#                 self.log.warning(f"Function '{fn}' not found for method '{method}'")
