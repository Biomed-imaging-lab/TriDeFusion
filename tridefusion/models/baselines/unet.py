...
class UpsamplingNearest2d(nn.Module):
    ...
    def forward(self, _x: Tensor) -> Tensor:
        return nn.functional.interpolate(_x, scale_factor=self.scale_factor, mode='nearest')
...
class UnetN2N(nn.Module):
    ...
    def forward(self, _x: Tensor) -> Tensor:
        out_pool1 = self.pool1(self.enc_relu1(self.enc_conv1(self.enc_relu0(self.enc_conv0(_x)))))
        ...
        out = self.dec_conv1c(self.dec_relu1b(self.dec_conv1b(self.dec_relu1a(self.dec_conv1a(torch.cat((out, _x), 1))))))
...
class UnetN2Nv2(nn.Module):
    ...
    def forward(self, _x: Tensor) -> Tensor:
        out_pool1 = self.pool1(self.enc_relu1(self.enc_bn1(self.enc_conv1(self.enc_relu0(self.enc_conv0(_x))))))
        ...
        out = self.dec_conv1c(self.dec_relu1b(self.dec_bn1b(self.dec_conv1b(self.dec_relu1a(self.dec_bn1a(self.dec_conv1a(torch.cat((out, _x), 1))))))))
        out = self.dec_act(out)
        out = torch.clamp(out, min=_x.min())
        return (out - out.min()) / (out.max() - out.min())
